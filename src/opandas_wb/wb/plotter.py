...
def multi_observables_plot(multiple_observables: List[Tuple[str, str, List[Observable]]]) -> Any:
    ...
    modify_plot(line_handles, ax, indic_countries, "Multiple Observables")
    return plt
...
def moving_avg(obs: List[Observable], w: int):
    ...
    modify_plot([line], ax, [(ind_id, country)], "Moving Average")
...
def diff_prime(obs: List[Observable], percentage=False):
    ...
    fig, ax = plt.subplots()
    line, = ax.plot(diff)
    modify_plot([line], ax, [(obs[0].indicator_id, obs[0].country)], title)
    return plt
...
def diff_prime_a_mano(obs: List[Observable]):
    ...
    modify_plot([line], ax, [(obs[0].indicator_id, obs[0].country)], "Prime Differenze housemade")
...
def diff_prime_percentuali_a_mano(obs: List[Observable]):
    ...
    modify_plot([line], ax, [(obs[0].indicator_id, obs[0].country)], "Prime Percentage Difference housemade")
    return plt
...
def regression_rect(obs: List[Observable]):
    ...
    modify_plot([regr_line, data_line], ax,
                [("Regression Rect", obs[0].country), (obs[0].indicator_id, obs[0].country)], "Regression Rect")
    return plt
...
def modify_plot(line_handles: List, ax: plt.axes, indicator_countries_label: List[Tuple[str, str]], title: str):
    """
